import { Experience, ExperienceAvailability, Registration } from "@prisma/client";
                ...
                availabilities.forEach(availability => {
                    if (availability.date !== null) {
                        ...
                        // The availability is scheduled for 3 days out
                            ...
                            sendDayRemindersToRegistrations(availability, "3 days from today");
                        }

                        // The availability is scheduled for 7 days out
                        else if (dateIsXDaysOut(availability.date, 7)) {
                            sendDayRemindersToRegistrations(availability, "a week from today");
                        }
                    }
                });
            }
...
function dateIsXDaysOut (date: Date, daysOut: number) {
    return (date >= add(startOfToday(), {days: daysOut}) && date <= add(startOfToday(), {days: daysOut + 1}));
}

function sendDayRemindersToRegistrations (availability: AvailabilityWithRegistrations, daysOutString: string) {
