    ...
    return (
        ...
        <div className="h-96 w-72 drop-shadow-xl rounded-2xl">
            ...
            <div className='absolute top-4 left-4 text-white'>
                <Image src={profile_pic} alt="Profile Picture Anonymous" width={40} className='border-white border rounded-full'/>
            </div>
            <div className='absolute top-3 right-3'>
                <h2 className='text-4xl text-white font-bold'>{experience.date.toLocaleDateString("en-US", dateDisplayOptions).toLocaleUpperCase()}</h2>
            </div>
            ...
            <div className='absolute bottom-0 w-full bg-white rounded-b-2xl h-36 p-3'>
                <p className='h-16 text-sm overflow-hidden'>{experience.description}</p>
                <div className='absolute bottom-5 left-5'>
                    <p className='font-bold text-xl'>${experience.price}</p>
                </div>
                <div className='absolute bottom-3 right-3'>
                    <button className='bg-amber-400 p-2 text-white drop-shadow-md rounded-lg' onClick={() => showModal()}>Details</button>
                </div>
            </div>
        </div>
